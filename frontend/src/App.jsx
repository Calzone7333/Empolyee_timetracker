  ...
  const updateData = async () => {
    try {
      ...
      if (currentId) {
        ...
        // 4. Productivity from summarized Apps & Sites (Active focus time)
        let prodSecs = 0, nonProdSecs = 0, neutralSecs = 0;
        let groupsInfo = { 'Productivity Apps': 0, 'Internet Browsing': 0, 'Others': 0 };

        (apps || []).forEach(a => {
          if (a.group === 'Productive') { prodSecs += (a.durationSecs || 0); groupsInfo['Productivity Apps'] += (a.durationSecs || 0); }
          else if (a.group === 'Non-Productive') nonProdSecs += (a.durationSecs || 0);
          else { neutralSecs += (a.durationSecs || 0); groupsInfo['Others'] += (a.durationSecs || 0); }
        });
        (websites || []).forEach(w => {
        ...
        const totalActiveSecs = prodSecs + nonProdSecs + neutralSecs || 1;
        const productivityData = [
          { name: 'Productive Hours', value: Math.round((prodSecs / totalActiveSecs) * 100), full: `${formatDur(prodSecs)}`, color: '#26a69a' },
          { name: 'Non-Productive Hours', value: Math.round((nonProdSecs / totalActiveSecs) * 100), full: `${formatDur(nonProdSecs)}`, color: '#ef5350' },
          { name: 'Neutral Hours', value: Math.round((neutralSecs / totalActiveSecs) * 100), full: `${formatDur(neutralSecs)}`, color: '#90a4ae' }
        ];

        // 5. Attendance segmentation logic
        let attendanceData = [];
        let timelineData = [];
        let totalIdleSecs = 0;
        let totalAwaySecs = 0;
        ...
        if (activityList.length > 0) {
          const sorted = [...activityList].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
          let segments = [];
          let currentSeg = [sorted[0]];
          ...
          for (let i = 1; i < sorted.length; i++) {
            ...
            if (gap > 10 * 60 * 1000) { // 10 min gap = Away
              totalAwaySecs += (gap / 1000);
              segments.push(currentSeg);
              currentSeg = [sorted[i]];
            } else {
              if (sorted[i].idleTime > 0) totalIdleSecs += (sorted[i].idleTime || 0);
              currentSeg.push(sorted[i]);
            }
          }
          segments.push(currentSeg);

          attendanceData = segments.map(seg => {
            const start = new Date(seg[0].timestamp);
            const end = new Date(seg[seg.length - 1].timestamp);
            const spentSecs = Math.floor((end - start) / 1000);
            return {
              start: start.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
              end: end.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
              spent: formatDur(spentSecs),
              activity: seg.length > 5 ? 'High' : 'Moderate',
          ...
          let vis = [];
          let curType = null, curWidth = 0;
          sorted.forEach(a => {
            const type = (a.application || '').toLowerCase().includes('code') || (a.website || '').toLowerCase().includes('google') ? 'productive' : 'neutral';
            if (type === curType) curWidth++;
            else {
              if (curType) vis.push({ type: curType, width: (curWidth / sorted.length) * 100 });
              curType = type; curWidth = 1;
            }
          });
          if (curType) vis.push({ type: curType, width: (curWidth / sorted.length) * 100 });
          timelineData.push({ date: formattedDate, day: 'Today', segments: vis, data: true });
        }

        const getStatsFor = (list) => {
          const acts = (list || []).filter(a => a.type === 'active');
          if (acts.length === 0) return { productive: '00:00', nonProductive: '00:00', neutral: '00:00', pPerc: 0, npPerc: 0, nPerc: 0 };
          let p = 0, np = 0, n = 0;
          acts.forEach(a => {
            const name = (a.application || '').toLowerCase();
            if (name.includes('code') || name.includes('studio') || name.includes('chrome')) p++;
            else if (name.includes('game') || name.includes('netflix')) np++;
            else n++;
          });
          const total = acts.length || 1;
          return {
            productive: formatDur(p * 10), nonProductive: formatDur(np * 10), neutral: formatDur(n * 10),
            pPerc: Math.round(p / total * 100), npPerc: Math.round(np / total * 100), nPerc: Math.round(n / total * 100)
          };
        };
        ...
        for (let i = 6; i >= 0; i--) {
          ...
          const dayActs = (weekActivity || []).filter(a => a.timestamp.startsWith(dStr) && a.type === 'active');
          weeklyBalance.push({
            date: d.toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }),
            actual: parseFloat((dayActs.length * 10 / 3600).toFixed(1)),
            expected: 8
          });
        }

        const activeCount = activityList.filter(a => a.type === 'active').length;
        setMonitoringData({
          user: currentUser,
          activeTime: formatDur(activeCount * 10),
          productivity: productivityData,
          attendance: attendanceData.length > 0 ? attendanceData : [{ start: '-', end: '-', spent: '00:00', activity: '-', status: 'Absent', type: 'absent' }],
          appGroups: Object.entries(groupsInfo).map(([name, val]) => ({ name, duration: formatDur(val), percentage: Math.round(val / (totalActiveSecs || 1) * 100) + '%' })),
          categories: productivityData.map(p => ({ name: p.name.replace(' Hours', ''), duration: p.full, percentage: p.value + '%' })),
          topApps: (apps || []).map(a => ({ name: a.app, value: a.percVal, full: a.dur, group: a.group, perc: a.perc, title: a.title })),
          topSites: (websites || []).map(w => ({ name: w.site, value: w.percVal, full: w.dur, group: w.group, perc: w.perc, fullTitle: w.fullTitle })),
          awayTime: formatDur(totalAwaySecs),
          idleTime: formatDur(totalIdleSecs),
          timeline: timelineData,
          screenshots: recentScreenshots,
          activityLevels: stats,
          rawActivities: activityList,
          teams: teamsData,
          departments: deptsData,
          trends: { today: { productive: formatDur(prodSecs), nonProductive: formatDur(nonProdSecs), neutral: formatDur(neutralSecs), pPerc: Math.round(prodSecs / totalActiveSecs * 100), npPerc: Math.round(nonProdSecs / totalActiveSecs * 100), nPerc: Math.round(neutralSecs / totalActiveSecs * 100) }, yesterday: yesterdayStats, week: weekStats },
          weeklyBalance
        });
      ...
      console.error('CRITICAL: updateData failed:', error);
    }
  };
                          ...
                          <tr key={i}>
                            <td>
                              <div
                                ...
                                style={{
                                  background: act.type === 'active' ? '#26a69a' : '#90a4ae',
                                  display: 'flex',
                                  alignItems: 'center',
                              ...
                              >
                                <ChevronDown size={10} color="white" style={{ transform: 'rotate(-135deg) translate(-1px, 1px)' }} />
                              </div>
                            </td>
                            ...
                            <td style={{ color: '#546e7a' }}>User</td>
                            <td style={{ fontWeight: '600' }}>{act.type === 'active' ? '00:00:10' : '00:00:00'}</td>
                            <td style={{ color: '#ef5350' }}>{act.idleTime || 0}s</td>
                            <td style={{ color: '#546e7a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '150px' }}>{act.application}</td>
                            <td style={{ color: '#546e7a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '150px' }}>{act.website || '-'}</td>
                            <td style={{ color: (act.idleTime || 0) > 120 ? '#ef5350' : '#90a4ae' }}>{(act.idleTime || 0) > 120 ? 'Yes' : 'No'}</td>
                          </tr>
                        ))
                      ) : (
